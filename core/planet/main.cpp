...
// camera
static Camera camera = Camera(glm::vec3(0.0f, 0.0f, 2.414f), float(SCR_WIDTH)/SCR_HEIGHT);
...
int main()
{
    ...
    // Prepare buffers
    std::shared_ptr<Mesh> star_mesh(new Icosphere(3));
    std::shared_ptr<std::vector<Shader>> star_shaders(new std::vector<Shader>);
    std::shared_ptr<std::vector<Shader>> planet_shaders(new std::vector<Shader>);

    star_shaders->push_back(Shader(FP("renderer/star.vert"),FP("renderer/star.frag")));
    star_shaders->push_back(Shader(FP("renderer/debug/wireframe.vert"),FP("renderer/debug/wireframe.frag"),FP("renderer/debug/wireframe.geom")));
    star_shaders->push_back(Shader(FP("renderer/debug/point.vert"),FP("renderer/debug/point.frag")));

    planet_shaders->push_back(Shader(FP("renderer/planet.vert"),FP("renderer/planet.frag")));
    planet_shaders->push_back(Shader(FP("renderer/debug/wireframe.vert"),FP("renderer/debug/wireframe.frag")));
    planet_shaders->push_back(Shader(FP("renderer/debug/point.vert"),FP("renderer/debug/point.frag")));

    Star sun("2k_sun.jpg", star_mesh, star_shaders);
    Planet earth( std::shared_ptr<Mesh>(new Icosphere(6)), planet_shaders);

    std::vector<Star> randomStars;
    for(int i = 0; i < 500; i++)
    {
        Star randomStar(star_mesh,star_shaders);
        randomStar.setRandom();
        randomStars.push_back(randomStar);
    }
