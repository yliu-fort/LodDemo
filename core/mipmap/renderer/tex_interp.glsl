...
void main()
{
    ...
    // coarse map ccd
    vec2 shlo = vec2(0.0,0.0);
    vec2 shhi = vec2(0.0,0.5);
    ...
    vec2 sh_pixel = shlo + (float(p+0.5)*(shhi-shlo))/float(HEIGHT_MAP_Y) + 0.25/float(HEIGHT_MAP_Y); // map to shlo->shhi
    ...
    //float height = texelFetch(largeMap,  ivec2(floor(shlo*HEIGHT_MAP_X) + p*(shhi-shlo)), 0 ).r;
    //if(p%2 == 1) {height = texture(largeMap,  sh_pixel ).r;}
