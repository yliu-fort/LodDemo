...
// camera
Camera camera(glm::vec3(0.5f, 1.0f, 2.0f), (float)SCR_WIDTH/SCR_HEIGHT);
...
// Shader
Shader MCShader, wireShader;

// Marching cube configuration
glm::ivec3 gridSize(128);
glm::vec3 voxelSize(glm::vec3(20.0f)/glm::vec3(gridSize));
float isoValue = 0.0f;
...
int main(int argc, char **argv)
{
    ...
    wireShader.reload_shader_program_from_files(FP("shader.box.vert"),FP("shader.box.frag"));
    ...
    //Datafield//
    glm::vec3 volTexSize = glm::vec3(16);
    ...
    for(int k=0; k<volTexSize.z; k++)
        for(int j=0; j<volTexSize.y; j++)
            for(int i=0; i<volTexSize.x; i++){

                dataField.push_back( 2*rand()/double(RAND_MAX)-1 );
                ...
                //dataField.push_back( tangle(2*i/(float)gridSize.x-1,2*j/(float)gridSize.y-1,2*k/(float)gridSize.z-1));

            }
    glTexImage3D( GL_TEXTURE_3D, 0, GL_R32F, volTexSize.x, volTexSize.y, volTexSize.z, 0,
                  GL_RED, GL_FLOAT, &dataField[0]);
    ...
    while( !glfwWindowShouldClose( window ) )
    {
        ...
        MCShader.setVec3("color",glm::vec3(0.2,0.2,0.7));

        MCShader.setMat4("projectionMatrix",camera.GetFrustumMatrix() );
        // Only translation and iso-scale is applicable

        MCShader.setMat4("model",glm::translate(glm::mat4(1),glm::vec3(0,-10,0)));
        ...
        wireShader.use();
        wireShader.setMat4("projectionMatrix",camera.GetFrustumMatrix() );
        renderCube();
