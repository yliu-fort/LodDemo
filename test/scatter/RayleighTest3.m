...
%% Sun angle, Sunrise and Sunset color
...
fCameraHeight = interp1([0 1],[fInnerRadius fOuterRadius],0.1);
fLightAngles = linspace(1,-1,512).';

for i = 1:numel(fLightAngles)
    fLightAngle = fLightAngles(i);
...
radiance = inScatter + groundAlbedo .* max(fLightAngles,0.0) .* transmittence * ESun;
